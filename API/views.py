    ...
    if user is not None:
            if user.is_active:
                ...
                try:
                    ...
                    auctionUser = Auction_User()
                return JSONResponse({'statusCode':status.HTTP_200_OK,'data':{'Message':'','Token':"Basic "+ base64.b64encode(hd_value),'id':user.id,'IsSuperUser':user.is_superuser,'firstName':user.first_name,'lastName':user.last_name,'username':username,'email':user.email,'address':auctionUser.address,'phone':auctionUser.phone}})
            else:
    ...
    if request.method == 'GET':
        ...
        if not User.objects.filter(username=username).exists():
            ...
            if len(InvalidFileds) > 0 :
                ...
                auctionUser.phone = phone
                auctionUser.save()
                return JsonResponse({'statusCode':status.HTTP_200_OK,'data':{'Message':'User Updated successfully'}})
...
def editAuction(request,auctionId):
    if Auction.exists(auctionId):
        ...
        if objAuction.seller== request.user:
            ...
            return JsonResponse({'statusCode':status.HTTP_400_BAD_REQUEST,'data':{'Message':'User can only edit own Auctions.'}})
    else:
        return JsonResponse({'statusCode':status.HTTP_400_BAD_REQUEST,'data':{'Message':'Auction Not Found.'}})